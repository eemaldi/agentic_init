...
class Config(Strict):
    ...
    enrich: bool = Field(False, description="Generate the agentinit-enrich skill for a Claude mapping pass.")
...
def load_config(root: Path) -> Config:
    ...
    if not path.exists():
        raise ConfigError(f"{CONFIG_FILE} not found in {root}. Run `agentinit init` first.")
    try:
