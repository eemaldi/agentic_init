...
from agentinit import __version__, catalog, doctor, enrich, wizard, writer
from agentinit.config import CONFIG_FILE, Config, ConfigError, Kind, load_config, save_config
from agentinit.detect import detect
from agentinit.engine import build
from agentinit.modules import bmad
...
Force = Annotated[bool, typer.Option("--force", help="Overwrite files modified locally or not managed by agentinit")]
...
def _next_steps(root: Path, config: Config) -> None:
    ...
    if config.enrich:
        steps.append("agentinit enrich  (let Claude map the codebase into the generated docs)")
...
def init(
    ...
    """Detect the project, ask a few questions, write agentinit.yaml and apply it."""
    if (root / CONFIG_FILE).exists():
        if yes:
            raise _fail(f"{CONFIG_FILE} exists; run `agentinit apply`, or delete it to start over.")
...
def apply_command(root: Root = Path("."), dry_run: DryRun = False, force: Force = False) -> None:
    """Regenerate the agent setup from agentinit.yaml."""
    try:
...
def enrich_command(root: Root = Path(".")) -> None:
    ...
    if not (root / ".claude" / "skills" / enrich.SKILL).exists():
        raise _fail(f"Set `enrich: true` in {CONFIG_FILE} and run `agentinit apply` first.")
...
@app.command()
def schema() -> None:
    """Print the JSON Schema for agentinit.yaml."""
    import json
