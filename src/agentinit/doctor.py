...
from agentinit.config import ConfigError
from agentinit.engine import Build, build
...
def _drift(root: Path, result: Build) -> list[Finding]:
    ...
    for path, entry in Lock.load(root).files.items():
        ...
        if entry["strategy"] == "owned" and file.exists() and digest(file.read_text()) != entry["hash"]:
            findings.append(Finding(Level.WARN, f"{path} modified locally; `agentinit apply --force` restores it"))
    pending = result.plan.pending
    if pending:
        findings.append(Finding(Level.WARN, f"{len(pending)} pending changes; run `agentinit diff`"))
    return findings or [Finding(Level.OK, "generated files match agentinit.yaml")]
