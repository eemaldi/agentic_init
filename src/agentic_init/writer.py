...
from agentic_init.merge import comment_style, merge, unmerge, upsert_blocks
...
def _owned(op: FileOp, before: str | None, entry: dict | None, force: bool) -> tuple[Change, dict | None]:
    ...
    if change.action == Action.UPDATE and not force:
        if entry is None:
            return Change(op.path, Action.SKIP, before, op.content, "exists and is not managed by agentic_init"), None
