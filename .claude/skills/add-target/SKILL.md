...
1. Read `src/agentic_init/targets/claude_code.py` and `src/agentic_init/blueprint.py`. Targets only translate; never add tool-specific logic to the resolver or catalog.
2. Create `src/agentic_init/targets/<tool>.py` with a class exposing `name` and `render(blueprint) -> list[FileOp]`.
...
3. Register it in `TARGETS` in `src/agentic_init/targets/__init__.py`.
