...
description: Add a new skill, agent, hook, rule, MCP server or docs seed to the agentinit catalog, wire it into presets and refresh snapshots. Use when extending what agentinit generates.
---

# Add a catalog component

1. Create `src/agentinit/catalog/<kind>/<name>/component.yaml` with a `description` of at least 8 words stating what it does and when to use it.
...
3. If it belongs in a preset, add it to `src/agentinit/presets.py` at the lowest level that needs it, or to a module in `src/agentinit/modules/__init__.py`.
