...
def test_init_yes_refuses_to_overwrite_existing_config(python_project):
    ...
    config = (python_project / "agentinit.yaml").read_text()

    result = CliRunner().invoke(app, ["init", str(python_project), "--yes"])

    assert result.exit_code == 1
    assert "agentinit apply" in result.output
    assert (python_project / "agentinit.yaml").read_text() == config
