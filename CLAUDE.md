...
## Architecture
`agentinit.yaml` → `resolver.py` (presets + modules + detection + Jinja catalog) → tool-agnostic `Blueprint` → `targets/*` render `FileOp`s → `writer.py` plans and applies them (owned files, managed blocks, JSON merge, seeds), tracking ownership in `.agentinit.lock`.
...
- Languages: python
- Package managers: uv
<!-- agentinit:end project -->

<!-- agentinit:begin commands -->
## Commands
...
<!-- agentinit:end commands -->

<!-- agentinit:begin workflow -->
## Working agreement
...
<!-- agentinit:end workflow -->

<!-- agentinit:begin verification -->
## Definition of done
